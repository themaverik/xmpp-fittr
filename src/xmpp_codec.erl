...
get_mod(<<"security-required">>,
        <<"urn:xmpp:jingle:errors:1">>) ->
    xep0166;
get_mod(<<"timestamp">>, <<"urn:xmpp:messagetime">>) ->
    timestamp;
get_mod(<<"TYPE">>, <<"vcard-temp">>) -> xep0054;
get_mod(<<"nickName">>, <<"vcard-temp">>) -> xep0054;
get_mod(<<"users">>, <<"jabber:iq:mod_broadcast">>) ->
    broadcast;
get_mod(<<"query">>,
...
get_mod(<<"channel">>, <<"urn:xmpp:mix:roster:0">>) ->
    xep0405;
get_mod(<<"chatcontent">>, <<"urn:xmpp:content">>) ->
    chatcontent;
...
get_mod(<<"data">>,
    ...
    xep0047;
get_mod(<<"logout">>, <<"urn:xmpp:receipts">>) ->
    logout;
...
get_mod(<<"actions">>,
    ...
    xep0050;
get_mod(<<"vCard">>, <<"jabber:iq:roster">>) -> rfc6121;
get_mod(<<"mechanisms">>,
        <<"urn:ietf:params:xml:ns:xmpp-sasl">>) ->
    rfc6120;
get_mod(<<"query">>,
...
get_mod(<<"participant">>,
        <<"jabber:iq:mod_broadcast">>) ->
    broadcast;
get_mod(<<"acknowledge">>, <<"urn:xmpp:receipts">>) ->
    acknowledge;
get_mod(<<"retract">>,
        <<"http://jabber.org/protocol/pubsub#event">>) ->
    xep0060;
...
get_mod(<<"city">>, <<"jabber:iq:register">>) ->
    xep0077;
get_mod(<<"status">>, <<"jabber:iq:roster">>) ->
    rfc6121;
...
get_mod(<<"invalid-options">>,
        <<"http://jabber.org/protocol/pubsub#errors">>) ->
    xep0060;
get_mod(<<"coverimg">>, <<"vcard-temp">>) -> xep0054;
...
get_mod(<<"content-type">>,
    ...
    xep0363;
get_mod(<<"delete_messages">>,
        <<"jabber:iq:del_message_activities">>) ->
    del_message_activities;
get_mod(<<"email">>, <<"jabber:iq:roster">>) -> rfc6121;
...
get_mod(<<"delegate">>, <<"urn:xmpp:delegation:1">>) ->
    xep0355;
get_mod(<<"mobileNumber">>, <<"jabber:iq:roster">>) ->
    rfc6121;
...
get_mod(<<"unsupported-applications">>,
        <<"urn:xmpp:jingle:1">>) ->
    xep0166;
get_mod(<<"recall">>, <<"urn:xmpp:receipts">>) ->
    recall;
...
get_mod(<<"decline">>, <<"urn:xmpp:jingle:1">>) ->
    xep0166;
get_mod(<<"delivered">>, <<"urn:xmpp:receipts">>) ->
    delivered;
...
get_mod(<<"failed-application">>,
        <<"urn:xmpp:jingle:1">>) ->
    xep0166;
get_mod(<<"query">>, <<"jabber:iq:mixgroupchat">>) ->
    mix_group_chat;
...
get_mod(<<"nickName">>, <<"jabber:iq:roster">>) ->
    rfc6121;
...
get_mod(<<"status">>, <<"vcard-temp">>) -> xep0054;
...
get_mod(<<"stream:stream">>,
        <<"jabber:component:accept">>) ->
    rfc6120;
get_mod(<<"query">>, <<"jabber:iq:user_fav">>) ->
    user_fav;
...
get_mod(<<"file-too-large">>,
        <<"urn:xmpp:jingle:apps:file-transfer:errors:0">>) ->
    xep0234;
get_mod(<<"reaction">>, <<"urn:xmpp:reaction:0">>) ->
    message_reaction;
get_mod(<<"call">>, <<"urn:xmpp:call">>) -> call;
...
get_mod(<<"query">>, <<"jabber:iq:mod_broadcast">>) ->
    broadcast;
...
get_mod(<<"unsubscribe">>, <<"urn:xmpp:mix:core:0">>) ->
    xep0369;
get_mod(<<"query">>, <<"jabber:iq:user_activities">>) ->
    user_activities;
...
get_mod(<<"reactions">>, <<"urn:xmpp:reactions:0">>) ->
    message_reactions;
...
get_mod(<<"image">>, <<"vcard-temp">>) -> xep0054;
get_mod(<<"subscription">>,
        <<"http://jabber.org/protocol/pubsub#owner">>) ->
    xep0060;
get_mod(<<"addresses">>,
...
get_mod(<<"activated">>,
        <<"urn:xmpp:jingle:transports:s5b:1">>) ->
    xep0260;
get_mod(<<"seen">>, <<"urn:xmpp:receipts">>) -> seen;
...
get_mod(<<"designation">>, <<"vcard-temp">>) -> xep0054;
...
get_mod(<<"hat">>,
        <<"xmpp:prosody.im/protocol/hats:1">>) ->
    xep0317;
get_mod(<<"mediacall">>, <<"urn:xmpp:call">>) -> call;
...
get_mod(<<"markable">>,
        <<"urn:xmpp:chat-markers:0">>) ->
    xep0333;
get_mod(<<"designation">>, <<"jabber:iq:roster">>) ->
    rfc6121;
...
get_mod(<<"security-error">>,
        <<"urn:xmpp:jingle:1">>) ->
    xep0166;
get_mod(<<"image">>, <<"jabber:iq:roster">>) -> rfc6121;
get_mod(<<"abort">>,
        <<"urn:ietf:params:xml:ns:xmpp-sasl">>) ->
    rfc6120;
...
get_mod(<<"iq">>, <<"jabber:component:accept">>) ->
    rfc6120;
get_mod(<<"mobileNumber">>, <<"vcard-temp">>) ->
    xep0054;
get_mod(<<"query">>,
        <<"http://jabber.org/protocol/muc#admin">>) ->
    xep0045;
...
get_mod(<<"leave">>, <<"urn:xmpp:mix:core:1">>) ->
    xep0369;
get_mod(<<"name">>, <<"jabber:iq:roster">>) -> rfc6121;
...
get_mod({roster_designation, _}) -> rfc6121;
get_mod({compression, _}) -> xep0138;
get_mod({vcard_designation, _}) -> xep0054;
...
get_mod({roster_mobilenumber, _}) -> rfc6121;
get_mod({vcard_mobile_number, _}) -> xep0054;
...
get_mod({group_query,
         ...
         _}) ->
    mix_group_chat;
get_mod({delete_messages, _}) -> del_message_activities;
...
get_mod({group_participant, _}) -> mix_group_chat;
get_mod({participant, _}) -> broadcast;
...
get_mod({roster_coverimg, _}) -> rfc6121;
...
get_mod({query, _, _, _, _, _}) -> broadcast;
get_mod({delivered, _, _}) -> delivered;
...
get_mod({service, _, _, _, _, _, _, _, _, _, _, _}) ->
    xep0215;
get_mod({timestampmsg, _, _}) -> timestamp;
get_mod({version, _, _, _}) -> xep0092;
get_mod({iq, _, _, _, _, _, _, _, _}) -> rfc6120;
get_mod({muc_subscription, _, _, _}) -> p1_mucsub;
get_mod({media_call_activities,
         _,
         _,
         _,
         _,
...
get_mod({buser, _}) -> broadcast;
get_mod({del_message_activities,
         _,
         _,
         _,
         _,
...
get_mod({group_subscriptions, _}) -> mix_group_chat;
get_mod({participants, _}) -> broadcast;
get_mod({sasl_success, _}) -> rfc6120;
get_mod({compress, _}) -> xep0138;
get_mod({xdata_field, _, _, _, _, _, _, _, _}) ->
    xep0004;
get_mod({ps_item, _, _, _, _, _, _, _}) -> xep0060;
...
get_mod({chatcontent, _, _, _, _}) -> chatcontent;
...
get_mod({users, _}) -> broadcast;
get_mod({roster_item, _, _, _, _, _, _, _}) -> rfc6121;
get_mod({roster_status, _}) -> rfc6121;
...
get_mod({roster_vcard, _, _, _, _, _, _, _, _, _, _}) ->
    rfc6121;
get_mod({roster_nickname, _}) -> rfc6121;
get_mod({roster_image, _}) -> rfc6121;
get_mod({roster_name, _}) -> rfc6121;
get_mod({private, _}) -> xep0049;
get_mod({sasl_challenge, _}) -> rfc6120;
get_mod({compress_failure, _}) -> xep0138;
get_mod({vcard_image, _}) -> xep0054;
...
get_mod({vcard_adr,
         ...
         _}) ->
    xep0054;
get_mod({vcard_coverimg, _}) -> xep0054;
...
get_mod({mix_client_join, _, _, _, _}) -> xep0405;
...
get_mod({acknowledge, _, _, _, _}) -> acknowledge;
...
get_mod({mam_query, _, _, _, _, _, _, _, _, _}) ->
    xep0313;
get_mod({mix_participant, _, _, _, _, _, _}) -> xep0369;
