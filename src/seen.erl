...
do_encode({seen_message, _, _, _} = Seen_message,
          TopXMLNS) ->
    encode_seen_message(Seen_message, TopXMLNS);
do_encode({seen_messages, _, _} = Seen_messages,
          TopXMLNS) ->
    encode_seen_messages(Seen_messages, TopXMLNS).

do_get_name({seen, _, _, _, _}) -> <<"seen">>;
do_get_name({seen_message, _, _, _}) ->
    <<"seen_message">>;
do_get_name({seen_messages, _, _}) ->
    <<"seen_messages">>.

do_get_ns({seen, _, _, _, _}) ->
    <<"urn:xmpp:receipts">>;
do_get_ns({seen_message, _, _, _}) ->
    <<"urn:xmpp:receipts:1">>;
do_get_ns({seen_messages, _, _}) ->
    <<"urn:xmpp:receipts:1">>.

pp(seen, 4) -> [id, time, chat_type, conversation_jid];
pp(seen_message, 3) -> [id, jid, time];
pp(seen_messages, 2) -> [chat_type, items];
pp(_, _) -> no.

records() ->
    [{seen, 4}, {seen_message, 3}, {seen_messages, 2}].
...
decode_seen_messages(__TopXMLNS, __Opts,
    ...
    Items = decode_seen_messages_els(__TopXMLNS,
                                     ...
                                     _els,
                                     []),
    Chat_type = decode_seen_messages_attrs(__TopXMLNS,
                                           _attrs,
                                           undefined),
    {seen_messages, Chat_type, Items}.
...
decode_seen_messages_attrs(__TopXMLNS,
    ...
    decode_seen_messages_attrs(__TopXMLNS, _attrs, _val);
...
encode_seen_messages({seen_messages, Chat_type, Items},
                     __TopXMLNS) ->
    __NewTopXMLNS =
    ...
    _els =
        lists:reverse('encode_seen_messages_$items'(Items,
                                                    __NewTopXMLNS,
                                                    [])),
    _attrs = encode_seen_messages_attr_chat_type(Chat_type,
...
decode_seen_message(__TopXMLNS, __Opts,
    ...
    {Id, Jid, Time} = decode_seen_message_attrs(__TopXMLNS,
                                                _attrs,
                                                undefined,
                                                undefined,
                                                undefined),
    {seen_message, Id, Jid, Time}.

decode_seen_message_attrs(__TopXMLNS,
                          [{<<"id">>, _val} | _attrs], _Id, Jid, Time) ->
    decode_seen_message_attrs(__TopXMLNS,
                              _attrs,
                              _val,
                              Jid,
...
decode_seen_message_attrs(__TopXMLNS, [_ | _attrs], Id,
    ...
    decode_seen_message_attrs(__TopXMLNS,
                              ...
                              Jid,
                              Time);
decode_seen_message_attrs(__TopXMLNS, [], Id, Jid,
                          Time) ->
    {decode_seen_message_attr_id(__TopXMLNS, Id),
     decode_seen_message_attr_jid(__TopXMLNS, Jid),
     decode_seen_message_attr_time(__TopXMLNS, Time)}.

encode_seen_message({seen_message, Id, Jid, Time},
                    __TopXMLNS) ->
    __NewTopXMLNS =
        xmpp_codec:choose_top_xmlns(<<"urn:xmpp:receipts:1">>,
                                    [],
    ...
    _attrs = encode_seen_message_attr_time(Time,
                                           encode_seen_message_attr_jid(Jid,
...
decode_seen_message_attr_jid(__TopXMLNS, undefined) ->
    <<>>;
decode_seen_message_attr_jid(__TopXMLNS, _val) -> _val.

encode_seen_message_attr_jid(<<>>, _acc) -> _acc;
encode_seen_message_attr_jid(_val, _acc) ->
    [{<<"jid">>, _val} | _acc].
